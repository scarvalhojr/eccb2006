...
In the next section, we review the Border Length Minimization Problem introduced by \citealp{HANNENHALLI02}, and define an extended model for evaluating microarray layouts. In section \ref{sec:previous_work} we briefly review exising placement strategies. In section \ref{sec:qap} we propose a new approach to the problem based on a quadratic assignment problem (QAP) formulation. We present the results of using a QAP heuritic algorithm to design small artificial chips in section \ref{sec:results}, compare its performance with the best known placement algorithm, and describe how this approach can be used to design larger microarrays.
...
We have run GRASP-PR as well as the best know placement algorithm, row-epitaxial (see section \ref{sec:placement_alg}), on several small random chips. The running time and the border length of the resulting layouts are shown on table \ref{tab:graspr_reptx}.

Our results show that GRASP-PR consistently produces layouts with lower border lengths than the row-epitaxial algorithm, although its running time increases at a much higher rate with larger chips. It is clear that GRASP-PR performs better on smaller regions. On a 6x6 chip GRASP-PR outperforms row-epitaxial by $2.5$ percentage points when compared to the initial random layout. On a 10x10 chip, this difference drops to $0.6$ percentage point.
...
It is interesting to extrapolate the times shown on table~\ref{tab:graspr_reptx} to predict the total time required to design the layout of a real chips such as the Affmetrix Human Genome U133 Plus 2.0 GeneChip\raisebox{.6ex}{\scriptsize \textregistered}, if we were to combine a partitioning algorithm with GRASP-PR.

If the partitioning produced 10x10 regions, $13 689$ sub-regions would be created from a 1164x1164 chip. Since each takes around 30 seconds to compute with GRASP-PR, the total time required for designing such a chip would be a little less than two hours (ignoring the time for the partitioning itself).

If the partitioning produced 12x12 regions, $9 409$ sub-regions would be created, but since GRASP-PR takes around 3 minutes to compute each of them, the total time would be more than 19 days. This may be too much time, although it is certainly possible to reduce the time of each GRASP-PR execution by running it on faster machines or using a parallel implementation; GRASP is known to be easily parallelized \citep{LI94}. Figure~\ref{fig:time_extrapolation} shows similar predictions based on our results with varying chip dimensions and partitioning sizes.

\begin{figure}
%\centerline{\includegraphics{fig01.eps}}
\caption{Predicted time to compute the layout of large microarrays using a ficticious partitioning algorithm and GRASP-PR on the resulting sub-regions based on our results. The chip dimensions are examples of existing GeneChip arrays. Each curve represents the maximum dimensions of sub-regions computed with GRASP-PR. The time required for the partitioning itself is ignored.}\label{fig:time_extrapolation}
\end{figure}

As mentioned in section \ref{sec:partition}, partitioning is a compromise in solution quality. However, it is not clear how the choice of the maximum recursion depth in the centroid-based quadrisection impairs the solution quality, and other partitioning strategies may exhibit have different characteristics.

We believe that solution quality is more important than the running time of a placement algorithm. Even if it takes a couple of days to complete, it is time well spent given that commercial microarrays are likely to be produced in large scale. This is specially true when we consider the time required for the whole design process of a microarray chip. Even customer designed chips that usually have only a few produced units are likely to find a few spare hours in the design process for computing a better layout.

TODO: it may also be used as an optimization algorithm if modified to take into account the border around the optimized region.
