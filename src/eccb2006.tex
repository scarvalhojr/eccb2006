...
The \emph{border length} of a mask~$\mu_{k}$ is defined as the number of borders shared by masked and unmasked spots at masking step~$k$. The total border length of a given arrangement is the sum of border lengths over all masks.
...
With these observations in mind, we define the conflict index~$\kappa(s)$ of a spot~$s$ whose probes of length~$\ell_{s}$ are synthesized in $\mu$~masking steps as follows. First we define a distance-dependent weighting function, $\delta(s,s',k)$, that accounts for observation a) above:
...
The distance-dependent weighthing function~$\delta$ is as suggested in \citealp{KAHNG03_1}. Our position-dependent weights~$\omega$, however, are different. Instead of using $\sqrt{\lambda(s,k)}$, we decided to use an exponential function. The reason is that the chances of a successful hybridization between a probe and its target is related to the free energy of the formed duplex, which is exponential in the number of consecutive paired bases. The range of values for both~$\delta$ and~$\omega$ on a typical Affymetrix chip are illustrated in Figure~\ref{fig:conflit_index_values}.
...
\end{equation}
%%
and our problem stated in equation \ref{eq:ci_min} turns into
%%
\begin{equation}
\label{eq:ci_min_qap}
...
%%
\begin{equation}
\mathcal{C}_{j}(i) := (d(i,j))^{-2} \cdot \sum_{k=1}^{\mu} \omega(i,k) \cdot \phi(j,k),
\end{equation}
%%
where
%%
...
\phi(j,k) :=
        \left\{
                \begin{array}{ll}
                        1 & \mbox{if spot $j$ is unmasked at step $k$}, \\
                        0 & \mbox{otherwise}, \\
                \end{array}
        \right.
\end{equation}
...
d_{ij} =
	\left\{
                \begin{array}{ll}
                        (d(i,j))^{-2} & \mbox{if spot $i$ is ``near'' spot $j$}, \\
                        0 & \mbox{otherwise}, \\
                \end{array}
	\right.
\end{equation}
%%
where ``near'' means that spot~$i$ is at most three cells away from~$j$.
...
Due to the large number of probes on industrial microarrays, we cannot use GRASP-PR (or any other QAP method) to design the layout of real chips. However, it is certainly possible to use it to design small regions of a chip. This is interesting because we can combine GRASP-PR with a partitioning strategy such as the centroid-based quadrisection described in section \ref{sec:partition}.
